...
function CompanySearch() {
    ...
    const decodedSearchData = decodeURIComponent(searchData);
    ...
    const fetchdata =  async ()=>{
      ...
      try{
        const {data} = await axios.get(`http://localhost:8000/search/${decodedSearchData}`)
    ...
    <label
        ...
        <div key={data._id} class="bg-gradient-to-br from-gray-900 to-gray-900 via-purple-500 to-indigo-800 bg-gradient-stops-0 bg-gradient-stops-50 bg-gradient-stops-100   bg-opacity-10 rounded-lg overflow-hidden shadow-2xl xl:w-1/5 lg:w-1/4 md:w-1/3 sm:w-1/2 mx-4 mb-6 hover:translate-y-[-10px] transition duration-300">
          ...
          <li class="flex items-center py-3 text-sm">
            <span>Name</span>
            <span class="ml-auto"><span class="rounded-full bg-purple-800 py-1 px-2 text-xs font-medium text-white">{data.companyName}</span></span>
          </li>
          <li class="flex items-center py-3 text-sm">
            <span>Location</span>
	...
	<div class="flex items-center">
      ...
      <div class="my-20 mx-auto flex flex-col items-center ml-14 lg:ml-[37.5%]">
        <h1 class="text-[120px] font-extrabold text-purple-600">404</h1>
        <p class="text-2xl font-medium text-purple-600 mb-6">No Result Found</p>
        <Link to="/companies"
            class="px-4 py-2 font-medium text-white bg-purple-600 rounded-md hover:bg-purple-800 transition-all duration-200 ease-in-out">
            Go Home
        </Link>
    </div>
