...
function AdminCompanies() {
  axios.defaults.withCredentials = true;
    ...
    const fetchData = async () => {
        try {
            const { data } = await axios.get("http://localhost:8000/companydata");
            console.log(data);
            setCompanyData(data); // Store data in state variable
        } catch (e) {
            console.log(e);
        }
    };
 ...
 <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
   <thead class="bg-gray-50">
     <tr>
       ...
       <th scope="col" class="px-8 py-4 font-medium text-gray-900">Status</th>
   
       <th scope="col" class="px-6 py-4 font-medium text-gray-900">Public Key</th>
  ...
  <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
    <div key={data._id} class="relative h-10 w-10">
      <img
        ...
        src={"http://localhost:8000/upload/"+data.logo}
        alt=""
      />
    ...
    <div class="text-sm">
      ...
      <div class="text-gray-400">{data.companyEmail}</div>
    </div>
  </th>
  <td class="px-6 py-4">
    <span
