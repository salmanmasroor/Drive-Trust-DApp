...
function UserRegistration() {
    ...
    const [previewSrc, setPreviewSrc] = useState('userprofile1.png');
  ...
  const handle = async (e) => {
    ...
    try {
        const {data} = await axios.post("http://localhost:8000/registration", formData, {
          headers: {
        ...
        if(data.errors){
            for(var i=0;i<data.errors.length;i++){
                if(data.errors[i].path === "username"){
                    setUsernameError(data.errors[i].msg)
                }
                else if(data.errors[i].path === "email"){
                    setEmailError(data.errors[i].msg)
                }
                else if(data.errors[i].path === "password"){
                    setPasswordError(data.errors[i].msg)
                }
            }
          ...
          setImageError(data.imageError)
            ...
            localStorage.setItem("userreg",data.userreg)
            navigate("/otp")
        }
    ...
    <div class="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style={{ maxWidth: '1000px' }}>
                    ...
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            ...
                            <div class="flex">
                                ...
                                <input type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="John" value={username} onChange={usernameChange} required/>
                            </div>
                            <p className={`text-red-500 ml-1`}>{usernameError}</p>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            ...
                            <div class="flex">
                                ...
                                <input type="email" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="johnsmith@gmail.com" value={email} onChange={emailChange} required/>
                            </div>
                            <p className={`text-red-500 ml-1`}>{emailError}</p>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-6">
                            ...
                            <div class="flex">
                                ...
                                <input type="password" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************" value={password} onChange={passwordChange} required/>
                            </div>
                            <p className={`text-red-500 ml-1`}>{passwordError}</p>
                        </div>
                    </div>
                    <div class="flex ">
      ...
      <label className="block">
        ...
        />
                    ...
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            <button class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">SIGN UP</button>
                        </div>
                    </div>
                    <div class="flex justify-center -mx-3">
                    <Link to="/login" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Log In</Link>   
                    </div>
                </div>
                </form>
            </div>
